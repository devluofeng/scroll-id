...
async function getRegistrarEntry(name) {
  ...
  if (nameArray.length > 3 || nameArray[1] !== 'scroll') {
    return {}
  }
...
async function getDNSEntryDetails(name) {
  ...
  if (nameArray.length !== 2 || nameArray[1] === 'scroll') return {}
...
function adjustForShortNames(node) {
  ...
  // return original node if is subdomain or not scroll
  if (nameArray.length > 2 || parent !== 'scroll' || label.length > 6) return node
...
const resolvers = {
  Query: {
    publicResolver: async () => {
      try {
        const ens = getENS()
        const resolver = await ens.getAddress('resolver.scroll')
        return {
          address: resolver,
          __typename: 'Resolver'
        }
    ...
    getResolverMigrationInfo: async (_, { name, resolver }) => {
      ...
      async function calculateIsPublicResolverReady() {
        const publicResolver = await ens.getAddress('resolver.scroll')
